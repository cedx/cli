...
export class DbEngineCommand
	...
	run: (engine) ->
		...
		await @_alterTable table, engine for table from tables.flat() when table.engine.toLowerCase() isnt normalizedEngine
...
export default (args) ->
	...
	switch
		...
		when not positionals.length then throw Error 'The required argument "engine" is missing.'
		when not values.dsn then throw Error 'The required option "--dsn" is missing.'

	new DbEngineCommand(values).run positionals[0]
