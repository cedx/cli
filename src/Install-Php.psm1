...
function Install-Php {
	...
	Write-Verbose "Fetching the list of PHP releases..."
	...
	Write-Verbose "Stopping the IIS web server..."
	Stop-Service W3SVC

	Write-Verbose "Extracting file ""$file"" into directory ""$Path""..."
	Expand-Archive $outputFile $Path -Force

	Write-Verbose "Starting the IIS web server..."
	Start-Service W3SVC

	if ($RegisterEventSource) {
		Write-Verbose "Registering the PHP interpreter with the event log..."
